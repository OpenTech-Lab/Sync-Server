...
pub fn update_news(
    ...
    let mut conn = pool.get()?;
    let affected = diesel::update(news_dsl::server_news.find(news_id))
        .set((
            news_dsl::title.eq(normalized_title.to_string()),
            news_dsl::summary.eq(normalized_summary),
            news_dsl::markdown_content.eq(normalized_markdown.to_string()),
            news_dsl::updated_at.eq(chrono::Utc::now()),
        ))
        .execute(&mut conn)?;
...
pub fn delete_news(pool: &Pool, news_id: Uuid) -> Result<ServerNews, AppError> {
    let mut conn = pool.get()?;
    let existing = news_dsl::server_news
        .find(news_id)
        .select(ServerNews::as_select())
        .first::<ServerNews>(&mut conn)
        .optional()?
    ...
    diesel::delete(news_dsl::server_news.find(news_id)).execute(&mut conn)?;
    Ok(existing)
}
